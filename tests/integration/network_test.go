...
var _ = Describe("Network Integration", Ordered, func() {
	...
	Describe("C-Chain Data Import", func() {
		...
		It("should export 7777 accounts for X-Chain funding", func() {
			...
			cmd := exec.Command("go", "run",
				...
				"--db-path", filepath.Join(genesisDir, "chaindata/lux-7777/db"),
	...
	Describe("7777 Dev Mode", func() {
		...
		It("should run 7777 chain in dev mode", func() {
			...
			pebbleDBPath := filepath.Join(genesisDir, "pebbledb/lux-7777")
			...
			cmd := exec.Command(luxdPath,
				...
				"--network-id=7777",
				"--db-dir", pebbleDBPath,
				"--chain-config-dir", filepath.Join(genesisDir, "configs/lux-7777"),
				"--http-port=9630",
				"--staking-port=9631",
			)
