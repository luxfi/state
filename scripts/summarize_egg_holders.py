#!/usr/bin/env python3
"""
Summarize EGG NFT holders from the provided data
"""

import json
from collections import defaultdict

# Known EGG holders from the data
egg_holders = {
    ...
    "0x66f3b12167edd9347c58": 2,  # Partial address
...
total_eggs = sum(egg_holders.values())
total_zoo = total_eggs * 4_200_000

# Distribution analysis
distribution = defaultdict(int)
for addr, count in egg_holders.items():
    if count == 1:
        distribution["1 EGG"] += 1
    elif 2 <= count <= 5:
        distribution["2-5 EGGs"] += 1
    elif 6 <= count <= 10:
        distribution["6-10 EGGs"] += 1
    elif 11 <= count <= 20:
        distribution["11-20 EGGs"] += 1
    elif count >= 100:
        distribution["100+ EGGs"] += 1
    else:
        distribution[">20 EGGs"] += 1

# Print summary
print("EGG NFT Holdings Summary")
print("=" * 60)
print(f"Total EGGs: {total_eggs:,}")
print(f"Total ZOO Value: {total_zoo:,}")
print(f"Unique Holders: {len(egg_holders)}")
print()

print("Distribution:")
print("-" * 40)
for tier, count in sorted(distribution.items()):
    print(f"{tier:15} {count:5} holders")
print()

print("Top 20 Holders:")
print("-" * 60)
print(f"{'Address':42} {'EGGs':>6} {'ZOO':>15}")
print("-" * 60)

# Sort by count descending
sorted_holders = sorted(egg_holders.items(), key=lambda x: (-x[1], x[0]))
for i, (addr, count) in enumerate(sorted_holders[:20]):
    zoo_amount = count * 4_200_000
    print(f"{addr:42} {count:6} {zoo_amount:15,}")
...
print("-" * 60)
special = {
    "0x28dad8427f127664365109c4a9406c8bc7844718": "Treasury/Marketplace (1300 EGGs)",
    "0x95a7b934860942e903c47d85041e263ea9167de8": "Zach (160 EGGs)", 
    "0xb58bab9ea70256195ba46243787b5685475ad137": "Largest holder (120 EGGs)",
    "0x9882fcf8dcf9bbef9306bc5294911964a0320137": "Large holder (30 EGGs)",
    "0xffdb31285961d44d40c404566e9de9080b1abd50": "OTC (2 EGGs)",
    "0xc06c7c6ec618de992d597d8e347669ea44ede2bc": "Jules (2 EGGs)",
    "0x6762ff916de1b315da56f4fa7b78f39aa60d9f4c": "Sean (1 EGG)",
}

for addr, desc in special.items():
    if addr in egg_holders:
        print(f"{addr}: {desc}")
...
output = {
    "summary": {
        ...
        "total_zoo_value": total_zoo,
        "unique_holders": len(egg_holders)
    },
    ...
    "holders": [
        ...
        for addr, count in sorted_holders
    ]
}

# Save to file
with open("egg_holders_summary.json", "w") as f:
    json.dump(output, f, indent=2)
    
print(f"\nSaved detailed summary to egg_holders_summary.json")